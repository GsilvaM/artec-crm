...
| `crm_internal.auvo_inbox_items` | Nao | Caixa de Entrada Auvo (`/api/auvo-inbox/*`) | Ativa desde 2026-07-21. Populada automaticamente a partir de eventos `SESSION_*` do webhook. Acesso via API restrito a `auvo_inbox:read`/`auvo_inbox:write` (gestor e atendimento). |
