...
const PAGES: Array<{ label: string; linkName: string | RegExp; urlPattern: RegExp }> = [
  ...
  { label: "Proximas Acoes", linkName: /Pr[oó]ximas A[cç][oõ]es/i, urlPattern: /\/proximas-acoes$/ },
];

for (const viewport of VIEWPORTS) {
  test.describe(`Responsividade sem overflow horizontal acidental - ${viewport.name}`, () => {
    // Definido via test.use em vez de page.setViewportSize em runtime para
    // manter a avaliacao inicial de @media/CSSOM consistente no Chromium.
